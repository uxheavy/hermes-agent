...
class TestRunConversation:
    ...
    def test_code_mode_rejects_direct_plane_operation_model_call(self, agent):
        ...
        with (
            ...
            result = agent.run_conversation("perform the Code Mode task")

        assert result["completed"] is True
        assert result["final_response"] == "done"
        mock_handle_function_call.assert_not_called()

    def test_code_mode_recalls_final_text_then_allows_code_and_publish_under_auto(self, agent):
        self._setup_agent(agent)
        agent.valid_tool_names.update({"plane_execute_typescript", "plane_publish"})
        agent._plane_first_required_tool = "plane_execute_typescript"
        agent._plane_first_required_tool_retries = 0
        agent.client.chat.completions.create.side_effect = [
            _mock_response(content="ordinary final before code", finish_reason="stop"),
            _mock_response(
                content="",
                finish_reason="tool_calls",
                tool_calls=[
                    _mock_tool_call(
                        ...
                        arguments='{"typescript_source":"export default () => 1;"}',
                        call_id="code",
                    )
                ],
            ),
        ...
        assert result["final_response"] == "done"
        assert calls == ["plane_execute_typescript", "plane_publish"]
        assert agent.client.chat.completions.create.call_count == 4
        assert [call.kwargs.get("tool_choice", "auto") for call in agent.client.chat.completions.create.call_args_list] == [
            "auto", "auto", "auto", "auto"
        ]

    def test_code_mode_repeated_final_text_fails_with_bounded_required_tool_reason(self, agent):
        self._setup_agent(agent)
        agent._plane_first_required_tool = "plane_execute_typescript"
        agent._plane_first_required_tool_retries = 0
        agent.client.chat.completions.create.side_effect = [
            _mock_response(content="ordinary final", finish_reason="stop"),
            _mock_response(content="ordinary final again", finish_reason="stop"),
            _mock_response(content="ordinary final once more", finish_reason="stop"),
        ]
        with (
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
