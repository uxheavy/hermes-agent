...
def test_code_mode_only_exposes_only_typescript_and_publication_tools():
    standard = SimpleNamespace(model_toolset="standard")
    code_mode = SimpleNamespace(model_toolset="code_mode_only")

    assert _plane_model_toolsets(code_mode) == (
        PLANE_PUBLICATION_TOOLSET,
        PLANE_CODE_MODE_TOOLSET,
    )
    assert _plane_model_toolsets(standard) == (
        PLANE_OPERATION_TOOLSET,
        PLANE_PUBLICATION_TOOLSET,
        PLANE_CODE_MODE_TOOLSET,
    )
    ...
    install_plane_tools()
    code_mode_tools = set(
        resolve_multiple_toolsets(list(_plane_model_toolsets(code_mode)))
    )
