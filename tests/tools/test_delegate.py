...
class TestFallbackModelInheritance(unittest.TestCase):
    ...
    def test_child_inherits_private_http_client_factory(self):
        """In-process children stay on the parent's provider-egress seam."""
        parent = _make_mock_parent(depth=0)
        factory = lambda: object()
        parent._http_client_factory = factory

        with patch("run_agent.AIAgent") as MockAgent:
            MockAgent.return_value = MagicMock()
            _build_child_agent(
                task_index=0,
                goal="test HTTP factory inheritance",
                context=None,
                toolsets=None,
                model=None,
                max_iterations=10,
